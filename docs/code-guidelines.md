...
domain 모듈에 있는 QueryDSL Projection 용도의 Class 라면 `@QueryProjection` 을 사용한다.
domain 모듈이 아닐때 만 `Projections.constructor()` 를 사용한다.

```java
//BAD
package com.example.domain.projectiondto;

public class SomeProjectionDto {
    private String name;
    private int age;
}
...
@QueryProjection
public record SomeProjectionDto(
        String name,
        int age
) { 
    @QueryProjection
    public SomeProjectionDto {}
}
